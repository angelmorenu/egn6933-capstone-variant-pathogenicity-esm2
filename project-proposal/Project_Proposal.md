...
The features used for model training will come from protein language model embeddings. In the current workflow, the project uses Dr. Fan’s lab-provided aligned, precomputed ESM2 embeddings as the primary feature source; **computing and caching ESM2-style embeddings from protein sequence context is treated as an optional/future extension** if needed to reproduce features end-to-end from public inputs (Rives et al., 2021; Lin et al., 2023).
...
To contextualize the performance of the ESM2 embedding-based models, the project will include a **comparison with existing variant pathogenicity predictors** such as PolyPhen-2, SIFT, or PrimateAI scores (if available for the curated dataset). This benchmarking step will help establish whether the protein language model approach offers competitive or complementary performance relative to established methods. Comparisons will be conducted on the same held-out test set using matched metrics (AUROC, AUPRC) and will be reported alongside the primary model results.
...
Although the primary goal is predictive performance, the project will include a lightweight, stakeholder-friendly visualization of the embedding feature space (e.g., **UMAP or t-SNE**) to illustrate whether pathogenic vs. benign variants show separable structure in the ESM2 embedding representation. This is intended as an explanatory diagnostic (not a mechanistic claim), and will be computed on held-out data to avoid over-interpretation.
...
Weeks 5–8 focus on feature engineering and baseline model development. Embedding vectors will be prepared for all variants (using aligned, precomputed embeddings in the current workflow; optional embedding generation is a future extension). Logistic Regression and Random Forest classifiers will be trained on the embedding features, and initial performance estimates using AUROC and AUPRC will be computed. Iterative analysis of class imbalance effects will inform threshold tuning and the selection of decision operating points.
...
As part of Weeks 9–12, the project will also run a **homology/leakage audit** (screening for highly similar proteins across splits) and generate a compact **UMAP/t-SNE visualization** of the embedding space colored by label and split to support communication with non-ML stakeholders. Additionally, the project will **benchmark against existing variant pathogenicity predictors** (e.g., PolyPhen-2, SIFT, PrimateAI) on the same test set to contextualize ESM2-based model performance relative to established methods.
